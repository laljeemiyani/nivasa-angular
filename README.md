...
cd ../frontend
cp .env.example .env       # Windows: copy .env.example .env
# Edit frontend/.env if needed (defaults are fine for local dev)
```

> **Note:** Both backend and frontend will show clear terminal errors if `.env` is missing.
...
- `npm run start`: Start the Angular dev server (port 4200)
...
├── frontend/          # Frontend code (Angular 19)
│   ├── public/        # Static files
